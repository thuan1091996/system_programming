...
portTASK_FUNCTION( vTask1, pvParameters ) 
{
	...
	while( 1 ) 
	{
		if(xQueueReceive(xled_control_queue, &sw_pressed, 0) == pdTRUE)
		{
			...
			if(xQueueSend(xlcd_display_queue, &led_msg_2lcd, 0) != pdTRUE)
			{
...
portTASK_FUNCTION( vTask2, pvParameters )
{
	...
	while( 1 )
	{
		if(xQueueReceive(xled_control_queue, &lcd_msg_recv, 0) == pdTRUE)
		{
...
portTASK_FUNCTION( vTask3, pvParameters )
{
	...
	while( 1 )
	{
		...
		if(xQueueSend(xlcd_display_queue, &thermo_msg_2lcd, 0) != pdTRUE)
		{
...
void button_init(void)
{
    ...
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = HARDWARE_INT_MIN_PRO+1;
